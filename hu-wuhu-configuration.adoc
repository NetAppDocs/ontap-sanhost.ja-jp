...
[NOTE]
====
Windows Host Utilities 7.2 をインストールした後、LimTransferSize パラメータの値を変更する必要があります。<<parameter-72,NVMEパラメータ>>を参照してください。
...
[[parameter-72]]Windows Host Utilities インストーラーは、インストール中に選択した内容に基づいて、Windows レジストリ値と iSCSI および NVMe HBA パラメーターを自動的に設定します。これらの値とオペレーティングシステムのバージョンを確認してください。
