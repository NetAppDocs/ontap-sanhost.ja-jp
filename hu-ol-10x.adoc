...
summary: Oracle Linux 10.xホストにLinux Host Utilitiesをインストールして、ONTAPのLUNでFCPおよびiSCSIプロトコル操作を管理します。 
---
= ONTAPストレージを使用したFCPおよびiSCSI用のOracle Linux 10.xの構成
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilitiesソフトウェアは、ONTAPストレージに接続されているLinuxホストの管理および診断ツールを提供します。Oracle Linux 10.xホストにLinux Host Utilitiesをインストールすると、Host Utilitiesを使用してONTAP LUNでのFCPおよびiSCSIプロトコル操作の管理を支援できます。
...
link:hu_luhu_71.html["ONTAPストレージ用のLinuxホストユーティリティ7.1をインストールする"]。
...
Oracle Linux 10.xのマルチパスを使用してONTAP LUNを管理できます。
