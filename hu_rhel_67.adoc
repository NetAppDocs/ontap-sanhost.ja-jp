...
summary: ONTAP で Red Hat Enterprise Linux 6.7 を使用する方法について説明します 
---
= Red Hat Enterprise Linux 6.7とONTAPの併用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてRed Hat Enterprise Linux 6.7を設定できます。
...
Red Hat Enterprise Linux （ RHEL ） 6.7 の場合は、 /etc/multipath.conf ファイルが存在する必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 6.7 には、 ONTAP LUN を認識して正しく管理するために必要なすべての設定が含まれています。ALUA ハンドラを有効にするには、次の手順を実行します。
...
RHEL 6.7 OS は、 ONTAP LUN を認識し、すべての設定パラメータを ASA 構成と非 ASA 構成の両方に対して自動的に正しく設定するようにコンパイルされています。
...
ONTAPリリースのRHEL 6.7に関する既知の問題はありません。
