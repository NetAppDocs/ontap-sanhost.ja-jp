...
summary: SUSE Linux Enterprise Server 15にLinux Host Utilitiesをインストールして、ONTAP LUNでFCPおよびiSCSIプロトコルの処理を管理します。 
---
= ONTAPストレージを使用したFCPおよびiSCSI用SUSE Linux Enterprise Server 15の設定
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilitiesソフトウェアは、ONTAPストレージに接続されたLinuxホスト用の管理ツールと診断ツールを提供します。SUSE Linux Enterprise Server 15ホストにLinux Host Utilitiesをインストールすると、Host Utilitiesを使用してONTAP LUNでのFCPおよびiSCSIプロトコルの処理を管理できます。
...
SUSE Linux Enterprise Server 15でマルチパスを使用して、ONTAP LUNを管理できます。
...
SUSE Linux Enterprise Server 15 with ONTAPストレージリリースには、次の既知の問題があります。
