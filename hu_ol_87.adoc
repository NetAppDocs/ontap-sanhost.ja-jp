...
summary: ONTAPでOracle Linux 8.7を使用する方法について説明します 
---
= 『Using Oracle Linux 8.7 with NetApp ONTAP』を参照してください
:toc: macro
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
...
NOTE: 1 つの LUN へのパスを余分に使用しないでください。パスは4つまでにする必要があります。8 個を超えるパスがストレージ障害時に原因パスの問題になる可能性があります。
...
NOTE: Oracle Linux 8.7 RedHat Enterprise Kernel（RHCK）を設定するには、を使用します link:hu_rhel_87.html#recommended-settings["推奨設定"] Red Hat Enterprise Linux（RHEL）8.7の場合。



== KVM設定

推奨設定を使用してKernel-Based Virtual Machine（KVM）を設定することもできます。LUNはハイパーバイザーにマッピングされるため、KVMの設定に必要な変更はありません。



== 既知の問題

Oracle Linux 8.7に関する既知の問題はありません。
...
Automatic Storage Management（ASM）ミラーリングでは、ASMが問題を認識して別の障害グループにスイッチオーバーできるように、Linuxマルチパス設定の変更が必要になる場合があります。ONTAP 上のほとんどの ASM 構成では、外部冗長性が使用されます。つまり、データ保護は外部アレイによって提供され、 ASM はデータをミラーリングしません。一部のサイトでは、通常の冗長性を備えた ASM を使用して、通常は異なるサイト間で双方向ミラーリングを提供しています。詳細については、「ONTAP 上のOracleデータベース」を参照してください。
