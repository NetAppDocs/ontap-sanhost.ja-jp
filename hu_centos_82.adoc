...
summary: ONTAP LUN を使用した FCP および iSCSI プロトコル操作を管理するには、CentOS 8.2 ホストに Linux ホスト ユーティリティをインストールします。 
---
= ONTAPストレージ用のFCPとiSCSI用にCentOS 8.2を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilitiesソフトウェアは、ONTAPストレージに接続されたLinuxホスト用の管理ツールと診断ツールを提供します。CentOS 8.2 ホストに Linux ホスト ユーティリティをインストールすると、ホスト ユーティリティを使用して ONTAP LUN での FCP および iSCSI プロトコル操作を管理できるようになります。
...
CentOS 8.2 のマルチパスを使用して ONTAP LUN を管理できます。
...
ONTAP ストレージを使用した FCP および iSCSI の CentOS 8.2 には既知の問題はありません。

CentOS Red Hat互換カーネルの既知の問題については、 link:hu_rhel_82.html#known-problems-and-limitations["既知の問題"] Red Hat Enterprise Linux (RHEL) 8.2 用。
