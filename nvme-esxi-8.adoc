...
* ONTAP 9.16.1 以降、新しく作成されたすべての NVMe ネームスペースに対して、デフォルトでスペース割り当てが有効になっています。
* ONTAP 9.9.1 P3以降、NVMe/FCプロトコルはESXi 8でサポートされています。
* ONTAP 9.10.1 以降、NVMe/TCP プロトコルは ONTAP でサポートされています。
...
| link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1420654["1420654"^] | ONTAPバージョン9.9.1でNVMe/FCプロトコルを使用するとONTAPノードが動作しなくなる | ONTAP 9.9.1では、NVMeの「abort」コマンドのサポートが導入されました。ONTAPがパートナーコマンドを待機しているNVMe融合コマンドを中止する「abort」コマンドを受信すると、ONTAPノードの停止が発生します。この問題は、NVMe融合コマンド（ESXなど）とファイバチャネル（FC）トランスポートを使用するホストでのみ発生します。 


| link:https://jira.ngage.netapp.com/browse/CONTAPEXT-1126["CONTAPEXT-1126"^] | vNVMeアダプタを使用するLinux VMで[All Paths Down（APD；すべてのパスが停止）]ウィンドウが長くなると、I/Oエラーが発生します  a| 
Linux VMがvSphere 8.x以降を実行し、仮想NVMe（vNVMe）アダプタを使用している場合、vNVMeの再試行操作がデフォルトで無効になっているため、I/Oエラーが発生します。古いカーネルを実行しているLinux VMが、全パスダウン（APD）時や高負荷のI/O時に中断されるのを避けるため、VMwareはvNVMe再試行操作を無効にするための調整可能な機能「VSCSIDisableNvmeRetry」を導入しました。
