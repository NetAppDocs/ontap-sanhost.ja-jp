...
---
= ONTAPストレージで NVMe-oF 用に RHEL 8.4 を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
RHEL 8.4 用の NVMe over Fabrics (NVMe-oF) ホストを構成する方法を学習します。詳細なサポートと機能情報については、link:hu-nvme-index.html["NVME-oFの概要"^] 。

*RHEL 8.4 の NVMe-oF には、次の既知の制限があります。*

* NVMe-oF プロトコルを使用した SAN ブートは現在サポートされていません。
* RHEL 8.4 の NVMe-oF ホストでは、カーネル内 NVMe マルチパスはデフォルトで無効になっているため、手動で有効にする必要があります。
...
.手順
. サーバーに RHEL 8.4 をインストールします。インストールが完了したら、必要な RHEL 8.4 カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
...
----
. RHEL 8.4ホストでは、次のhostnqn文字列を確認します。 `/etc/nvme/hostnqn` :
+
[source, cli]
----
