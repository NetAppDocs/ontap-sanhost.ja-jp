...
summary: SUSE Linux Enterprise Server 15 SP7にLinux Host Utilitiesをインストールして、ONTAP LUNでFCPおよびiSCSIプロトコルの処理を管理します。 
---
= ONTAPストレージを使用してFCPおよびiSCSI用にSUSE Linux Enterprise Server 15 SPxを構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux ホスト ユーティリティ ソフトウェアは、 ONTAPストレージに接続された Linux ホスト用の管理および診断ツールを提供します。SUSE Linux Enterprise Server 15 SPx ホストに Linux ホスト ユーティリティをインストールすると、ホスト ユーティリティを使用してONTAP LUN での FCP および iSCSI プロトコル操作を管理できるようになります。
...
SUSE Linux Enterprise Server 15 SPx のマルチパスを使用して、 ONTAP LUN を管理できます。
...
ONTAPストレージを搭載した SUSE Linux Enterprise Server 15 SPx には、次の既知の問題があります。
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1308744["1308744"^] | SUSE Linux Enterprise Server 15S P2 OSのインストール完了後、SANからのiSCSIブートが静的IP設定で起動しない  a| 
静的IP設定を使用したSUSE Linux Enterprise Server 15 SP2 OSのインストールが完了すると、iSCSIサンブートLUNの起動に失敗する。ブートアップ障害は、静的 IP 設定で毎回発生します。これにより、次のエラーメッセージが表示され、サーバが起動プロセスの続行を拒否しています。
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1154309["1154309"^] | 20 個を超える LUN がマッピングされている SLES 15 ホストは、リブート後に保守モードになる場合があります | 20 個を超える LUN がマッピングされている SLES 15 ホストは、リブート後に保守モードになる場合があります。次のメッセージが表示され、メンテナンスモードがシングルユーザモードになります。
