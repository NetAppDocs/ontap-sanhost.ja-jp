...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてOracle Linux 6.10を設定できます。
...
NOTE: Oracle Linux 6.10 Red Hat Enterprise Kernel（RHCK）を設定するには、link:hu_rhel_610.html#recommended-settings["推奨設定"]for Red Hat Enterprise Linux（RHEL）6.10を使用します。



== 既知の問題

ONTAPリリースのOracle Linux 6.10では、既知の問題はありません。
