...
summary: ONTAP で Red Hat Enterprise Linux 8.2 を使用する方法について説明します 
---
...
[role="lead"]
ONTAPのSANホストの設定を使用して、ONTAPをターゲットとしてRed Hat Enterprise Linux 8.2を設定できます。
...
Red Hat Enterprise Linux（RHEL）8.2の場合 `/etc/multipath.conf` ファイルが存在している必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 8.2は、ONTAP LUNを認識して適切に管理するために必要なすべての設定が組み込まれています。
...
ONTAPリリースのRHEL 8.2に関する既知の問題はありません。
