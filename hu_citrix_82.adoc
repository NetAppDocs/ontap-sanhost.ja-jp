...
----
====
. マルチパスを初めて設定する場合は、推奨設定を読み込むためにマルチパスサービスを有効にして起動する必要がある場合があります。
+
[source, cli]
----
...
ホストが他のベンダーの LUN に接続されていて、マルチパスパラメータ設定のいずれかが上書きされている場合は、既存のスタンザを変更するか、ONTAP LUN に特化して適用される `/etc/multipath/conf.d/custom.conf`ファイルに新しいスタンザを追加して修正する必要があります。ホストに適用される変更点を確認するには、ファイル内を下にスクロールする必要があります。パラメーターを修正しないと、ONTAP LUN が期待どおりに動作しない可能性があります。
