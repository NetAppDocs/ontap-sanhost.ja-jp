...
summary: ONTAP でRed Hat Enterprise Linux 8.7を使用する方法について説明します 
---
= NetApp ONTAP でRed Hat Enterprise Linux 8.7を使用します
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Red Hat Enterprise Linux（RHEL）8.7の場合 `/etc/multipath.conf` ファイルが存在している必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 8.7は、ONTAP LUNを認識して正しく管理するために必要なすべての設定でコンパイルされます。
...
RHEL 8.7 OSは、ONTAP LUNを認識するようにコンパイルされ、ASA 構成と非ASA構成の両方に対してすべての設定パラメータが自動的に正しく設定されます。
...
| 1501048 | FCトランスポートレイヤタイムアウトパラメータ `dev_loss_tmo` リモートSCSIターゲットポートの場合、負の値（-1）が報告される |  `dev_loss_tmo` タイムアウトパラメータは、リモートポートが失われないようにFC転送を絶縁するデフォルトの秒数を表します。この値を超えると、SCSIターゲットがホストから削除されます。このパラメータ値は正の整数値である必要があります。ただし、RHEL 8.7では、 `sysfs` Filesystemはの負の値（-1）を返します `dev_loss_tmo` SCSIデバイスのタイムアウトパラメータ。次に、を確認するコマンドラインの例を示します `dev_loss_tmo` 稼働中のホストのパラメータ：
`# cat /sys/devices/pci*/*/*/host*/rport*/fc_remote_ports/rport*/dev_loss_tmo
4294967295`。RHEL 8.7には、この問題 の回避策 はありません。 | link:https://bugzilla.redhat.com/show_bug.cgi?id=2124297["2124297"^] 
|===
