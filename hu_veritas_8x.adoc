...
[role="lead"]
FC、FCoE、およびiSCSIの各プロトコルを使用するRed Hat Enterprise LinuxおよびOracle Linux（RHCKベース）プラットフォーム用のVeritas Infoscale Storage Foundation 8シリーズリリースでは、ONTAP SANホストの構成設定を使用できます。
