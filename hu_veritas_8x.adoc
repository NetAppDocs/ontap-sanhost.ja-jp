...
---
= VERITAS Infosscale 8 for LinuxとNetApp ONTAP の併用
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
このドキュメントでは、FC、FCoE、およびiSCSIプロトコルを搭載したRed Hat Enterprise LinuxおよびOracle Linux（RHCKベース）プラットフォーム用のVERITAS Infoscaleストレージ基盤8シリーズのONTAP SANホスト構成設定について説明します。
