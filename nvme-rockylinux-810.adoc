...
---
= ONTAPストレージでNVMe-oF用にRocky Linux 8.10を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 8.10 用に NVMe over Fabrics (NVMe-oF) ホストを構成する方法を学びます。詳細なサポートと機能情報については、 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAPのサポートと機能"]。

*Rocky Linux 8.10 の NVMe-oF には、次の既知の制限があります。*

* NVMe-oF プロトコルを使用した SAN ブートは現在サポートされていません。
* Rocky Linux 8.10 の NVMe-oF ホストでは、カーネル内 NVMe マルチパスはデフォルトで無効になっているため、手動で有効にする必要があります。
...
.手順
. サーバーに Rocky Linux 8.10 をインストールします。インストールが完了したら、必要な Rocky Linux 8.10 カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Rocky Linux 8.10 NVMe-oFホストは重複した永続的検出コントローラを作成します | NVMe-oF ホストでは、「nvme discover -p」コマンドを使用して永続検出コントローラ (PDC) を作成できます。このコマンドを使用する場合、イニシエーターとターゲットの組み合わせごとに 1 つの PDC のみを作成する必要があります。ただし、NVMe-oF ホストで Rocky Linux 8.10 を実行している場合は、「nvme discover -p」を実行するたびに重複した PDC が作成されます。これにより、ホストとターゲットの両方でリソースが不必要に使用されることになります。 
|===