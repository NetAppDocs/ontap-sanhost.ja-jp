...
トラブルシューティングを始める前に、以下の要件に準拠した構成を実行していることを確認してください。 https://mysupport.netapp.com/matrix/#welcome["Interoperability Matrix Tool"^] 仕様を確認してから次の手順に進み、ホスト側の問題をデバッグします。


NOTE: トラブルシューティングの手順は、AIX、ESXi、Proxmox、および Windows ホストには適用されません。
...
問題が解決しない場合は、以下のファイルとコマンド出力を収集してlink:https://mysupport.netapp.com["ネットアップサポート"^]に問い合わせ、さらなるトリアージを依頼してください：
