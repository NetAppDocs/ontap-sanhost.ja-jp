...
summary: ONTAP で CentOS 6.9 を使用する方法について説明します 
---
= CentOS 6.9をONTAPで使用
:hardbreaks:
:toclevels: 1
...
[role="lead"]
ONTAP SANホストの設定を使用して、CentOS 6.9でONTAPをターゲットとして設定できます。
...
CentOS 6.9 の場合は '/etc/multipath.conf ファイルが存在している必要がありますが ' ファイルに特定の変更を加える必要はありませんCentOS 6.9 は、 ONTAP LUN を認識して正しく管理するために必要なすべての設定でコンパイルされています。
...
CentOS 6.9 OS は、 ONTAP LUN を認識するようにコンパイルされ、 ASA 構成と非 ASA 構成の両方ですべての構成パラメータを自動的に正しく設定します。
...
CentOS （ Red Hat 互換カーネル）の既知の問題については、を参照してください link:hu_rhel_69.html#known-problems-and-limitations["既知の問題"] Red Hat Enterprise Linux （ RHEL ） 6.9 の場合
