...
NOTE: Oracle Linux 8.8 Red Hat Compatible Kernel（RHCK）を設定するには、for Red Hat Enterprise Linux（RHEL）8.8を使用しlink:hu_rhel_88.html#recommended-settings["推奨設定"]ます。
...
ONTAPリリースのOracle Linux 8.8では、既知の問題はありません。
