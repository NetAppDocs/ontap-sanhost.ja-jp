...
summary: ONTAP で Rocky Linux 8.5 用の NVMe-oF ホストを構成する方法 
---
= ONTAPストレージ用にNVMe-oFを使用してRocky Linux 8.5を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Rocky Linux 8.5 ホストは、非対称名前空間アクセス (ANA) を備えた NVMe/FC および NVMe/TCP プロトコルをサポートします。ANAは、iSCSI および FCP 環境における非対称論理ユニットアクセス (ALUA) マルチパスに相当し、カーネル内 NVMe マルチパス機能を使用して実装されます。

サポートされる構成の詳細については、を参照してくださいlink:https://mysupport.netapp.com/matrix/["Interoperability Matrix Tool"^]。

.このタスクについて
Rocky Linux 8.5 の NVMe-oF ホスト構成では、次のサポートと機能を使用できます。設定プロセスを開始する前に、既知の制限事項も確認しておく必要があります。
...
* 既知の制限事項：
+
** Rocky Linux 8.5 NVMe-oF ホストでは、カーネル内 NVMe マルチパスはデフォルトで無効になっています。そのため、手動で有効にする必要があります。
** Rocky Linux 8.5 ホストでは、未解決の問題のため、NVMe/TCP はテクノロジー プレビュー機能です。詳細については、を参照して https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.5_release_notes/index#technology-preview_file-systems-and-storage["Rocky Linux 8.5 リリースノート"^] ください。
...
サポートされている Rocky Linux 8.5 ソフトウェアの最小バージョンを検証するには、次の手順に従います。

.手順
. サーバーにRocky Linux 8.5をインストールします。インストールが完了したら、指定されたRocky Linux 8.5カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
...
ONTAP リリースの Rocky Linux 8.5 上の NVMe-oF ホスト構成には既知の問題はありません。
