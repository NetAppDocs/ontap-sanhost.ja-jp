...
---
= Red Hat Enterprise Linux 7.9とONTAPの併用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてRed Hat Enterprise Linux 7.9を設定できます。
...
Red Hat Enterprise Linux （ RHEL ） 7.9 の場合は、 /etc/multipath.conf ファイルが存在する必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 7.9 には、 ONTAP LUN を認識して正しく管理するために必要なすべての設定が含まれます。
...
RHEL 7.9 OS は、 ONTAP LUN を認識するようにコンパイルされ、 ASA 構成と非 ASA 構成の両方に対してすべての設定パラメータが自動的に正しく設定されます。
...
RHEL 7.9 with ONTAPリリースには、次の既知の問題があります。
