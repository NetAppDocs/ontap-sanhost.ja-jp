...
** NetAppは、Oracle が全体的な構成をサポートし、LUN に直接アクセスできるパーティションがサポートされている構成でlink:https://imt.netapp.com/matrix/#welcome["IMT にログインします。"^]にリストされている場合、Solaris 仮想化テクノロジーの使用をサポートします。これには、ルートコンテナ、LDOM I/O ドメイン、および NPIV を使用して LUN にアクセスする LDOM が含まれます。
...
+
.. Solaris ホストに最新の Support Repository Update（SRU）がインストールされていることを確認します：
+
[source, cli]
----
pkg info entire
