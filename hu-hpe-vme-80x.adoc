...
ホストが他のベンダーの LUN に接続されていて、マルチパスパラメータ設定のいずれかが上書きされている場合は、ONTAP LUN に特化して適用される `multipath.conf`ファイルにスタンザを追加して修正する必要があります。ファイルを下にスクロールして、これらのスタンザを追加してください。パラメーターを修正しないと、ONTAP LUN が期待どおりに動作しない可能性があります。
