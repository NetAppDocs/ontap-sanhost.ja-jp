...
summary: ONTAPを使用した Rocky Linux 9.x 向け NVMe-oF ホストの設定方法 
---
= ONTAPストレージでNVMe-oF用にRocky Linux 9.xを構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 9.x 用に NVMe over Fabrics (NVMe-oF) ホストを構成する方法を学習します。詳細なサポートと機能情報については、 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAPのサポートと機能"]。

Rocky Linux 9.x の NVMe-oF には、次の既知の制限があります。
...
.手順
. サーバーに Rocky Linux 9.x をインストールします。インストールが完了したら、必要な Rocky Linux 9.x カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
...
Rocky Linux 9x ホストとONTAPコントローラ間の NVMe/TCP 経由の安全なインバンド認証がサポートされます。
...
----
. Rocky Linux 9.x ホストの dhchap キーを生成します。
+
