...
summary: ONTAP で Red Hat Enterprise Linux 7.4 を使用する方法について説明します 
---
= Red Hat Enterprise Linux 7.4とONTAPの併用
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてRed Hat Enterprise Linux 7.4を設定できます。
...
Red Hat Enterprise Linux （ RHEL ） 7.4 の場合は、 /etc/multipath.conf ファイルが存在する必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 7.4 は ONTAP LUN を認識して適切に管理するために必要なすべての設定で構成されています
...
RHEL 7.4 OS は ONTAP LUN を認識するようにコンパイルされ、 ASA 構成と非 ASA 構成の両方ですべての構成パラメータが自動的に正しく設定されます。
...
RHEL 7.4 with ONTAPリリースには、次の既知の問題があります。
