...
summary: SUSE Linux Enterprise Server 15 SP6とONTAPの使用方法について説明します。 
---
= SUSE Linux Enterprise Server 15 SP6とONTAPの併用
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP SANホストの設定を使用して、SUSE Linux Enterprise Server 15 SP6とONTAPをターゲットとして設定できます。
...
.必要なもの
ご使用の構成でSANブートがサポートされているlink:https://mysupport.netapp.com/matrix/imt.jsp?components=84067;&solution=1&isHWU&src=IMT["NetApp Interoperability Matrix Tool で確認できます"^]OS、HBA、HBAファームウェア、HBAブートBIOS、およびONTAPバージョンがサポートされていることを確認するには、を参照してください。
...
SUSE Linux Enterprise Server 15 SP6の場合は、 `/etc/multipath.conf`ファイルが存在している必要があります。SUSE Linux Enterprise Server 15 SP6は、ONTAP LUNを認識して正しく管理するために必要なすべての設定でコンパイルされているため、このファイルを変更する必要はありません。
...
SUSE Linux Enterprise Server 15 SP6 OSはONTAP LUNを認識し、ASA構成と非ASA構成の両方について、すべての設定パラメータを自動的に正しく設定します。次の推奨設定を使用して、ホスト構成のパフォーマンスをさらに最適化できます。
...
----
.例
次の例では、デバイスのWWIDを特定し、ファイルにを追加します `multipath.conf`。
