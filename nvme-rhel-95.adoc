...
Red Hat Enterprise Linux (RHEL) 9.5 用の NVMe over Fabrics (NVMe-oF) ホストを構成する方法を学習します。サポートと機能に関する詳しい情報については、link:hu-nvme-index.html["NVME-oFの概要"^] 。

RHEL 9.5 の NVMe-oF には既知の制限はありません。
...
.手順
. サーバーに RHEL 9.5 をインストールします。インストールが完了したら、必要な RHEL 9.5 カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
