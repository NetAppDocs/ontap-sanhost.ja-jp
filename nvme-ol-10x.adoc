...
summary: ONTAPを使用したOracle Linux 10のNVMe-oFホスト構成 
---
...
Oracle Linux 10.x 用の NVMe over Fabrics （NVMe-oF）ホストを構成する方法を学習します。詳細なサポートと機能情報については、link:nvme-ol-supported-features.html["Oracle Linux ONTAPのサポートと機能"]を参照してください。
...
----
. Oracle Linux 10.x ホストで、 `hostnqn` 文字列を `/etc/nvme/hostnqn`で確認します：
+
[source, cli]
----
...
Oracle Linux 10.x ホストと ONTAP コントローラ間の NVMe/TCP でセキュアなインバンド認証がサポートされています。
