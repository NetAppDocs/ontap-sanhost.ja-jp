...
summary: ONTAPでOracle Linux 9.3を使用する方法について説明します。 
---
= ONTAPでOracle Linux 9.3を使用
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてOracle Linux 9.3を設定できます。
...
OL 9.3では `/etc/multipath.conf`ファイルが存在している必要がありますが、ファイルに特定の変更を加える必要はありません。OL 9.3は、ONTAP LUNを認識して適切に管理するために必要なすべての設定でコンパイルされています。
...
Oracle Linux 9.3 OSでは、ONTAP LUNが認識され、ASA構成とASA以外の構成の両方について、すべての設定パラメータが自動的に正しく設定されます。次の推奨設定を使用して、ホスト構成のパフォーマンスをさらに最適化できます。
...
NOTE: Oracle Linux 9.3 Red Hat Enterprise Kernel（RHCK）を設定するには、link:hu_rhel_93.html#recommended-settings["推奨設定"]Red Hat Enterprise Linux（RHEL）9.3用を使用します。
...
Oracle Linux 9.3 with ONTAPリリースには、次の既知の問題があります。
