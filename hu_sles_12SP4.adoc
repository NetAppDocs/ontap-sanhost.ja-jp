...
summary: ONTAP LUN での FCP および iSCSI プロトコル操作を管理するには、SUSE Linux Enterprise Server 12 SP4 に Linux ホスト ユーティリティをインストールします。 
---
= ONTAPストレージを使用したFCPおよびiSCSI用SUSE Linux Enterprise Server 12 SP4の設定
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilitiesソフトウェアは、ONTAPストレージに接続されたLinuxホスト用の管理ツールと診断ツールを提供します。SUSE Linux Enterprise Server 12 SP4ホストにLinux Host Utilitiesをインストールすると、Host Utilitiesを使用してONTAP LUNでのFCPおよびiSCSIプロトコルの処理を管理できます。
...
SUSE Linux Enterprise Server 12 SP4でマルチパスを使用して、ONTAP LUNを管理できます。
