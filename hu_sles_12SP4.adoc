...
---
= NetApp ONTAP で SUSE Linux Enterprise Server 12 SP4 を使用する方法
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
SUSE Linux Enterprise Server 12 SP4 OS は、 ONTAP LUN を認識し、すべての構成パラメータを自動的に正しく設定するようにコンパイルされました。マルチパスデーモンを開始するには、「 multipath.conf 」ファイルが存在している必要がありますが、「 touch /etc/multipath.conf 」コマンドを使用して空のゼロバイトファイルを作成できます
...
SUSE Linux Enterprise Server 12 SP4 では、既知の問題はありません。
