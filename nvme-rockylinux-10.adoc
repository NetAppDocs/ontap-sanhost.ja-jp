...
Rocky Linux 10では、  `nvmefc-boot-connections.service`そして `nvmf-autoconnect.service` NVMe/FCに含まれるブートサービス `nvme-cli`パッケージはシステムの起動時に自動的に有効になります。起動が完了したら、サービスが有効になっていることを確認してください。
