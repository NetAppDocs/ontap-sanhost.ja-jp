...
summary: ONTAPストレージを搭載したProxmox VE 9.0用のNVMe-oFホストを構成する方法 
---
= ONTAPストレージでNVMe-oF用にProxmox VE 9.0を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Proxmox VE 9.0 ホストは、非対称名前空間アクセス (ANA) を備えた NVMe over Fibre Channel (NVMe/FC) および NVMe over TCP (NVMe/TCP) プロトコルをサポートします。ANA は、iSCSI および FCP 環境における非対称論理ユニット アクセス (ALUA) と同等のマルチパス機能を提供します。

Proxmox VE 9.0 用に NVMe over Fabrics (NVMe-oF) ホストを構成する方法を学びます。詳細なサポートと機能情報については、 link:nvme-proxmox-supported-features.html["ONTAPのサポートと機能"^]。


NOTE: Proxmox VE 9.0 は、NVMe-FC の SAN ブート構成をサポートしていません。



== ステップ1: ソフトウェアバージョンとNVMe構成を確認する

NVMe-oF 用にホストを構成するには、ホストと NVMe ソフトウェア パッケージをインストールし、マルチパスを有効にして、ホストの NQN 構成を確認する必要があります。

.手順
. サーバーに Proxmox VE 9.0 をインストールします。インストールが完了したら、必要な Proxmox VE 9.0 カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
