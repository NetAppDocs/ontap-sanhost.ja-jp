...
summary: ONTAP で Red Hat Enterprise Linux 7.3 を使用する方法について説明します 
---
= ONTAPでRed Hat Enterprise Linux 7.3を使用する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてRed Hat Enterprise Linux 7.3を設定できます。
...
Red Hat Enterprise Linux （ RHEL ） 7.3 では、 /etc/multipath.conf ファイルが存在する必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 7.3 には、 ONTAP LUN を認識して正しく管理するために必要なすべての設定が含まれています。
...
RHEL 7.3 OS は、 ONTAP LUN を認識するようにコンパイルされ、 ASA 構成と非 ASA 構成の両方に対してすべての設定パラメータが自動的に正しく設定されます。
...
ONTAPリリースのRHEL 7.3に関する既知の問題はありません。
