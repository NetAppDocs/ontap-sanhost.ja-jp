...
---
= ONTAPストレージでNVMe-oF用にRocky Linux 9.3を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Rocky Linux 9.3 用に NVMe over Fabrics (NVMe-oF) ホストを構成する方法を学びます。詳細なサポートと機能情報については、 link:nvme-rockylinux-supported-features.html["Rocky Linux ONTAPのサポートと機能"]。

*Rocky Linux 9.3 の NVMe-oF には、次の既知の制限があります。*
...
.手順
. サーバーに Rocky Linux 9.3 をインストールします。インストールが完了したら、必要な Rocky Linux 9.3 カーネルが実行されていることを確認します。
+
[source, cli]
----
uname -r
