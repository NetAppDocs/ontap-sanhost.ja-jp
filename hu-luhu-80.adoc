...
.このタスクについて
Linuxホストユーティリティをインストールしても、ホスト上の設定や動作は一切変更されません。
...
NOTE: Linux Host Utilities は、libHBAAPI.so ライブラリへのインストール依存関係があります。ホストが iSCSI 専用構成を想定している場合でも、上記に挙げたパッケージのいずれかをインストールする必要があります。あるいは、LUN およびパス管理には OS レベルのツールを使用することもできます。
