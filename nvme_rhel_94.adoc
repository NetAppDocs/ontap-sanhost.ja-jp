...
NOTE: RHEL 9.4以降では、NVMe/TCPのデフォルト設定 `ctrl_loss_tmo` タイムアウトがオフになっているため、再試行回数に制限はありません（無期限の再試行）。そのため、特定の `ctrl_loss_tmo` nvme connectコマンドまたはnvme connect-allコマンドを使用した場合のタイムアウト時間（オプション-l）。このデフォルトの動作では、パスで障害が発生してもNVMe/TCPコントローラはタイムアウトせず、無期限に接続されたままになります。
