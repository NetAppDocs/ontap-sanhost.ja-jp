...
---
= NetApp ONTAP で SUSE Linux Enterprise Server 12 SP2 を使用している
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
SUSE Linux Enterprise Server 12 SP2 OS は、 ONTAP LUN を認識し、すべての設定パラメータを正しく自動的に正しく設定するようにコンパイルされています。マルチパスデーモンを開始するには、「 multipath.conf 」ファイルが存在している必要がありますが、「 touch /etc/multipath.conf 」コマンドを使用して空のゼロバイトファイルを作成できます
...
SUSE Linux Enterprise Server 12 SP2 には既知の問題はありません。
