...
----
. RHELホストでは、次のhostnqn文字列を確認します。  `/etc/nvme/hostnqn` ：
+
[source, cli]
----
...
RHEL 9.6では、  `nvmefc-boot-connections.service`そして `nvmf-autoconnect.service` NVMe/FCに含まれるブートサービス `nvme-cli`パッケージはシステムの起動時に自動的に有効化されます。起動が完了したら、サービスが有効になっていることを確認してください。
