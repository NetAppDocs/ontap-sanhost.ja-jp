...
summary: ONTAP で Red Hat Enterprise Linux 8.4 を使用する方法について説明します 
---
= NetApp ONTAP で Red Hat Enterprise Linux 8.4 を使用する
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Red Hat Enterprise Linux （ RHEL ） 8.4 の場合、 /etc/multipath.conf ファイルが存在している必要がありますが、ファイルに特定の変更を加える必要はありません。RHEL 8.4 は、 ONTAP LUN を認識して正しく管理するために必要なすべての設定を含むようにコンパイルされています。
...
RHEL 8.4 OS は、 ONTAP LUN を認識するようにコンパイルされ、 ASA 構成と非 ASA 構成の両方に対してすべての設定パラメータが自動的に正しく設定されます。
