...
NOTE: Oracle Linux 8.0 Red Hat Compatible Kernel（RHCK）を設定するには、for Red Hat Enterprise Linux（RHEL）8.0を使用しますlink:hu_rhel_80.html#recommended-settings["推奨設定"]。
...
NOTE: Oracle Linux RHCKの既知の問題については、RHEL 8.0のを参照してくださいlink:hu_rhel_80.html#known-problems-and-limitations["既知の問題"]。
