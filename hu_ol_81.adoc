...
NOTE: Oracle Linux 8.1 Red Hat Enterprise Kernel（RHCK）を設定するには、for Red Hat Enterprise Linux（RHEL）8.1を使用しlink:hu_rhel_81.html#recommended-settings["推奨設定"]ます。
...
NOTE: Oracle Linux（Red Hat互換カーネル）の既知の問題については、for Red Hat Enterprise Linux（RHEL）8.1を参照してくださいlink:hu_rhel_81.html#known-problems-and-limitations["既知の問題"]。
