...
[role="lead"]
ONTAP SANホストの設定を使用して、ONTAPをターゲットとしてOracle Linux 8.5を設定できます。
...
NOTE: Oracle Linux 8.5 Red Hat Enterprise Kernel（RHCK）を設定するには、link:hu_rhel_85.html#recommended-settings["推奨設定"]for Red Hat Enterprise Linux（RHEL）8.5を使用します。
...
ONTAPリリースのOracle Linux 8.5に関する既知の問題はありません。
