...
.手順
. サーバにRHEL 8.6をインストールします。インストールが完了したら、指定した RHEL 8.6 カーネルを実行していることを確認します。サポートされているバージョンの最新のリストについては、を参照してくださいlink:https://mysupport.netapp.com/matrix/["Interoperability Matrix Tool"^]。
. インストールが完了したら、指定した RHEL 8.6 カーネルを実行していることを確認します。サポートされているバージョンの最新のリストについては、を参照してくださいlink:https://mysupport.netapp.com/matrix/["Interoperability Matrix Tool"^]。
+
例
...
--
RHEL 8.6カーネルに含まれているネイティブの受信トレイ `qla2xxx`ドライバには、アップストリームに関する最新の修正が適用されています。これらの修正は、ONTAPのサポートに不可欠です。
