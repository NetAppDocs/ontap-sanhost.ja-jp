...
summary: ONTAP LUN を使用した FCP および iSCSI プロトコル操作を管理するには、Rocky Linux 8.4 ホストに Linux ホスト ユーティリティをインストールします。 
---
= ONTAPストレージでFCPとiSCSI用にRocky Linux 8.4を構成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Linux Host Utilitiesソフトウェアは、ONTAPストレージに接続されたLinuxホスト用の管理ツールと診断ツールを提供します。Rocky Linux 8.4 ホストに Linux ホスト ユーティリティをインストールすると、ホスト ユーティリティを使用して ONTAP LUN での FCP および iSCSI プロトコル操作を管理できるようになります。
...
Rocky Linux 8.4 のマルチパスを使用して ONTAP LUN を管理できます。
