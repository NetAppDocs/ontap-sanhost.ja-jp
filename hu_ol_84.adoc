...
NOTE: Oracle Linux 8.4 Red Hat Enterprise Kernel（RHCK）を設定するには、for Red Hat Enterprise Linux（RHEL）8.4を使用しlink:hu_rhel_84.html#recommended-settings["推奨設定"]ます。
...
ONTAPリリースのOracle Linux 8.4では、既知の問題はありません。


NOTE: Oracle Linux（Red Hat互換カーネル）の既知の問題については、対応するRed Hat Enterprise Linuxリリースマニュアルのセクションを参照してくださいlink:hu_rhel_84.html#known-problems-and-limitations["既知の問題"]。
