...
NOTE: Oracle Linux 8.4 Red Hat Compatible Kernel（RHCK）を設定するには、for Red Hat Enterprise Linux（RHEL）8.4を使用しますlink:hu_rhel_84.html#recommended-settings["推奨設定"]。
...
NOTE: Oracle Linux RHCKの既知の問題については、対応するRHELリリースドキュメントのセクションを参照してくださいlink:hu_rhel_84.html#known-problems-and-limitations["既知の問題"]。
