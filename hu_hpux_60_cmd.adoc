...
summary: HP-UX Host Utilities 6.0をONTAPで使用する方法について説明します。 
---
= HP-UXホストユーティリティコマンドを使用してONTAPストレージ構成を確認します
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Host Utilities ツールを使用してONTAPストレージ構成をエンドツーエンドで検証するには、HP-UX Host Utilities 6.0 サンプル コマンド リファレンスを使用します。



== ホストにマッピングされているすべてのホストイニシエータを一覧表示

ホストにマップされているすべてのホスト イニシエーターのリストを取得します。
...
ホストにマップされているすべての LUN のリストを取得します。
...
特定の SVM からホストにマップされているすべての LUN のリストを取得します。
...
ホストにマップされた指定された LUN のすべての属性のリストを取得します。
...
指定されたホスト デバイス ファイル名でONTAP LUN 属性のリストを取得します。
...
ホストに接続されているすべての SVM ターゲット LIF WWPN のリストを取得します。
