...
[role="lead"]
FC、FCoE、iSCSIの各プロトコルを使用するRed Hat Enterprise LinuxおよびOracle Linux（Red Hat互換カーネルベース）プラットフォーム用のVeritas Infoscale Storage Foundation 7シリーズリリースでは、ONTAP SANホストの設定を使用できます。
