...
SnapMirror Business Continuity（SM-BC）環境で計画外のサイトフェイルオーバースイッチオーバーが発生したときにSolarisクライアントアプリケーションを無停止で実行できるようにするには、Solaris 11.4ホストで次の設定を行う必要があります。この設定は、フェールオーバーモジュール–を上書きします `f_tpgs` 矛盾を検出するコードパスが実行されないようにするため。
...
"NETAPP  LUN","f_tpgs"
----
. 使用 `devprop` および `mdb` オーバーライドパラメータが正常に適用されたことを確認するコマンド：
+
...
NOTE: `conf` がに追加されます `svl_fops_name` いつ `scsi-vhci-failover-override` が適用されました。追加情報 およびデフォルト設定への推奨される変更については、ネットアップの技術情報アーティクルを参照してください https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/Solaris_Host_support_recommended_settings_in_SnapMirror_Business_Continuity_(SM-BC)_configuration["Solaris ホストでは、 SnapMirror Business Continuity （ SM-BC ）構成での推奨設定がサポートされます"^]。
...
| 1362435 | HUK 6.2 および Solaris_11.4 FC ドライババインディングの変更 | Solaris 11.4 および HUK の推奨事項FC ドライバのバインドが SSD （ 4D ）から SD （ 4D ）に変更されました。「 sd.conf 」にある構成を「 sd.conf 」に移動します。詳細については、 Oracle ( ドキュメント ID 2595926.1) を参照してください。この動作は、新しくインストールされた Solaris 11.4 システムによって異なり、 11.3 以下のバージョンからアップグレードされます。 | （ドキュメント ID 2595926.1 ） 
