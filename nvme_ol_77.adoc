...
ONTAP 9.6以降では、Oracle Linux 7.7でNVMe/FCがサポートされます。Oracle Linux 7.7ホストでは、NVMeトラフィックとSCSIトラフィックの両方を、同じファイバチャネルイニシエータアダプタポート経由で実行できます。サポートされるFCアダプタとコントローラの一覧については、を参照してください link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] 。サポートされる構成の最新のリストについては、を参照してくださいlink:https://mysupport.netapp.com/matrix/["Interoperability Matrix Tool"^]。
...
----
. ホストをリブートします。
. 推奨されるBroadcom LPFCファームウェア、標準受信トレイドライバ、および送信トレイ自動接続パッケージバージョンを使用していることを確認します。サポートされているバージョンの一覧については、を参照してlink:https://mysupport.netapp.com/matrix/["Interoperability Matrix Tool"^]ください。
+
[listing]
----
