...
NOTE: NetApp AIX Host Utilitiesソフトウェアパッケージは、にあるlink:https://mysupport.netapp.com/site/products/all/details/hostutilities/downloads-tab/download/61343/7.0["ネットアップサポートサイト"^]圧縮されたtar.gzファイルで入手できます。AIX MPIOとNetApp ONTAPストレージを使用する際には、AIX Host Utilitiesキットをインストールする必要があります。
